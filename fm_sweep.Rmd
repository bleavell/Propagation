...
Sample rate = 250 kHz
Bit depth = 16 (PCM)
Duration = 1.002 seconds (0.5 sec silence -> 2 ms signal -> 0.5 sec silence)
...
## Oscillogram
``` {r oscillo}
oscillo(fmsweep, from = 0.500, to =0.502)
```

## Power spectrum
``` {r powerspec}
meanspec(fmsweep,  wl = 256, ovlp = 75, from = 0.498, to =0.504)
```

## Spectrogram
``` {r spectrogram}
spectro(fmsweep,  wl = 128, ovlp = 75, tlim = c(0.498,0.504))
```
